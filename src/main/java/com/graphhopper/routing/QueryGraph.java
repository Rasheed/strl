...
public class QueryGraph implements Graph {
	...
	private final static int VE_BASE = 0, VE_BASE_REV = 1, VE_ADJ = 2,
			VE_ADJ_REV = 3;
	...
	/**
	 * Convenient method to initialize this QueryGraph with the two specified
	 * query results.
	 */
	...
	public void lookup(List<QueryResult> resList) {
		if (isInitialized())
			throw new IllegalStateException(
					"Call lookup only once. Otherwise you'll have problems for queries sharing the same edge.");
		...
		TIntObjectMap<List<QueryResult>> edge2res = new TIntObjectHashMap<List<QueryResult>>(
				resList.size());

		// Phase 1
		// calculate snapped point and swap direction of closest edge if
		// necessary
		for (QueryResult res : resList) {
			// Do not create virtual node for a query result if it is directly
			// on a tower node or not found
			...
			if (closestEdge == null)
				throw new IllegalStateException(
						"Do not call QueryGraph.lookup with invalid QueryResult "
								+ res);

			int base = closestEdge.getBaseNode();

			// Force the identical direction for all closest edges.
			// It is important to sort multiple results for the same edge by its
			// wayIndex
			boolean doReverse = base > closestEdge.getAdjNode();
			if (base == closestEdge.getAdjNode()) {
				// check for special case #162 where adj == base and force
				// direction via latitude comparison
				PointList pl = closestEdge.fetchWayGeometry(0);
				if (pl.size() > 1)
					doReverse = pl.getLatitude(0) > pl
							.getLatitude(pl.size() - 1);
			}
			...
			if (doReverse) {
				...
				if (res.getWayIndex() < 0)
					throw new IllegalStateException(
							"Problem with wayIndex while reversing closest edge:"
									+ closestEdge + ", " + res);
			}
		...
		// 2. create virtual edges between virtual nodes and its neighbor
		// (virtual or normal nodes)
			...
			public boolean execute(List<QueryResult> results) {
				...
				Collections.sort(results, new Comparator<QueryResult>() {
					...
					public int compare(QueryResult o1, QueryResult o2) {
						...
						if (diff == 0) {
							// sort by distance from snappedPoint to
							// fullPL.get(wayIndex) if wayIndex is identical
							GHPoint p1 = o1.getSnappedPoint();
							GHPoint p2 = o2.getSnappedPoint();
							if (p1.equals(p2))
								return 0;

							double fromLat = fullPL.getLatitude(o1
									.getWayIndex());
							double fromLon = fullPL.getLongitude(o1
									.getWayIndex());
							if (distCalc.calcNormalizedDist(fromLat, fromLon,
									p1.lat, p1.lon) > distCalc
									.calcNormalizedDist(fromLat, fromLon,
											p2.lat, p2.lon))
								return 1;
							return -1;
						}
				...
				// Create base and adjacent PointLists for all none-equal
				// virtual nodes.
				// We do so via inserting them at the correct position of fullPL
				// and cutting the
				...
				for (int counter = 0; counter < results.size(); counter++) {
					...
					if (res.getClosestEdge().getBaseNode() != baseNode)
						throw new IllegalStateException(
								"Base nodes have to be identical but were not: "
										+ closestEdge + " vs "
										+ res.getClosestEdge());
					...
					createEdges(prevPoint, prevWayIndex, res.getSnappedPoint(),
							res.getWayIndex(), fullPL, closestEdge, prevNodeId,
							virtNodeId, reverseFlags, walkability);
				...
				// two edges between last result and adjacent node are still
				// missing
				createEdges(prevPoint, prevWayIndex,
						fullPL.toGHPoint(fullPL.getSize() - 1),
						fullPL.getSize() - 2, fullPL, closestEdge,
						virtNodeId - 1, adjNode, reverseFlags, walkability);
	...
	private void createEdges(GHPoint3D prevSnapped, int prevWayIndex,
		...
		VirtualEdgeIState baseEdge = new VirtualEdgeIState(virtEdgeId,
				prevNodeId, nodeId, baseDistance, closestEdge.getFlags(),
				closestEdge.getName(), basePoints, walkability);
		VirtualEdgeIState baseReverseEdge = new VirtualEdgeIState(virtEdgeId,
				nodeId, prevNodeId, baseDistance, reverseFlags,
				closestEdge.getName(), baseReversePoints, walkability);
	...
	public EdgeIteratorState getEdgeProps(int origEdgeId, int adjNode) {
		...
		if (eis2.getAdjNode() == adjNode)
			return eis2;
		throw new IllegalStateException("Edge " + origEdgeId
				+ " not found with adjNode:" + adjNode + ". found edges were:"
				+ eis + ", " + eis2);
	}
	...
	public EdgeExplorer createEdgeExplorer(final EdgeFilter edgeFilter) {
		if (!isInitialized())
			throw new IllegalStateException(
					"Call lookup before using this graph");

		// Iteration over virtual nodes needs to be thread safe if done from
		// different explorer
		// so we need to create the mapping on EVERY call!
		// This needs to be a HashMap (and cannot be an array) as we also need
		// to tweak edges for some mainNodes!
		// The more query points we have the more inefficient this map could be.
		// Hmmh.
		final TIntObjectMap<VirtualEdgeIterator> node2EdgeMap = new TIntObjectHashMap<VirtualEdgeIterator>(
				queryResults.size() * 3);

		final EdgeExplorer mainExplorer = mainGraph
				.createEdgeExplorer(edgeFilter);
		final TIntHashSet towerNodesToChange = new TIntHashSet(
				queryResults.size());

		// 1. virtualEdges should also get fresh EdgeIterators on every
		// createEdgeExplorer call!
		for (int i = 0; i < queryResults.size(); i++) {
			// create outgoing edges
			VirtualEdgeIterator virtEdgeIter = new VirtualEdgeIterator(2);
			EdgeIteratorState baseRevEdge = virtualEdges.get(i * 4
					+ VE_BASE_REV);
			...
			// replace edge list of neighboring tower nodes: a) add virtual
			// edges only and collect tower nodes where real edges will be added
			// in step 2.
			// base node
		...
		// 2. the connected tower nodes from mainGraph need fresh EdgeIterators
		// with possible fakes
		// where 'fresh' means independent of previous call and respecting the
		// edgeFilter
		// -> setup fake iterators of detected tower nodes (virtual edges are
		// already added)
	 ...
	 */
	private void addVirtualEdges(
			TIntObjectMap<VirtualEdgeIterator> node2EdgeMap, EdgeFilter filter,
			boolean base, int node, int virtNode) {
		...
		EdgeIteratorState edge = base ? virtualEdges
				.get(virtNode * 4 + VE_BASE) : virtualEdges.get(virtNode * 4
				+ VE_ADJ_REV);
		if (filter.accept(edge))
			existingIter.add(edge);
	}

	void fillVirtualEdges(TIntObjectMap<VirtualEdgeIterator> node2Edge,
			int towerNode, EdgeExplorer mainExpl) {
		if (towerNode >= mainNodes)
			throw new IllegalStateException("should not happen:" + towerNode
					+ ", " + node2Edge);

		VirtualEdgeIterator vIter = node2Edge.get(towerNode);
		TIntArrayList ignoreEdges = new TIntArrayList(vIter.count() * 2);
		while (vIter.next()) {
			EdgeIteratorState edge = queryResults.get(
					vIter.getAdjNode() - mainNodes).getClosestEdge();
	...
	@Override
	public EdgeIteratorState edge(int a, int b, double distance,
	...
	static class VirtualEdgeIterator implements EdgeIterator, EdgeSkipIterState {
		...
		public boolean isShortcut() {
			...
			return edge instanceof EdgeSkipIterState
					&& ((EdgeSkipIterState) edge).isShortcut();
		}

		@Override
		public double getWeight() {
			// will be called only from PreparationWeighting and if isShortcut
			// is true
	 ...
	 * are kept in memory.
	 */
	private static class VirtualEdgeIState implements EdgeIteratorState,
			EdgeSkipIterState {
