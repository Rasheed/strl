...
public class OSMReader implements DataReader
{
    ...
    void processWay( OSMWay way )
    {
            ...
            if (nodeFlags > 0)
            {
                if ((nodeFlags & wayFlags) > 0)
                {
                    ...
                    if (i > 0)
                    {
                        ...
                        createdEdges.addAll(addOSMWay(partIds, wayFlags, wayOsmId, walkability));
        ...
        if (lastBarrier >= 0)
        {
            if (lastBarrier < size - 1)
            {
                ...
                createdEdges.addAll(addOSMWay(partNodeIds, wayFlags, wayOsmId, walkability));
            }
        } else
        {
            // no barriers - simply add the whole way
            createdEdges.addAll(addOSMWay(way.getNodes(), wayFlags, wayOsmId, walkability));
        }
            ...
            for (int i = 0; i < osmNodeIds.size(); i++)
            {
                ...
                if (tmpNode == PILLAR_NODE)
                {
                    ...
                    if (!pointList.isEmpty() && lastInBoundsPillarNode > -TOWER_NODE)
                    {
                        ...
                        {
                            // TOWER node
                            newEdges.add(addEdge(firstNode, tmpNode, pointList, flags, wayOsmId, walkability));
                            pointList.clear();
    ...
    Collection<EdgeIteratorState> addBarrierEdge( long fromId, long toId, long flags, long nodeFlags, long wayOsmId )
    {
        ...
        return addOSMWay(barrierNodeIds, flags, wayOsmId, 0);
    }
